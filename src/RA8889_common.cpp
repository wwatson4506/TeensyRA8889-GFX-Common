...
//**************************************************************//
PROGMEM unsigned char gImage_busy_im[256] = {
...
//**************************************************************//
PROGMEM unsigned char gImage_no_im[256] = {
    /* 0X00,0X02,0X20,0X00,0X20,0X00, */
    0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA,
...
//**************************************************************//
PROGMEM unsigned char gImage_arrow_il[256] = {
    /* 0X00,0X02,0X20,0X00,0X20,0X00, */
...
//**************************************************************//
PROGMEM unsigned char gImage_pen_il[256] = {
    /* 0X00,0X02,0X20,0X00,0X20,0X00, */
    0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA, 0XAA,
...
void RA8889_common::RA8889_GFX(uint16_t buswidth) {
    _width = SCREEN_WIDTH;
    _height = SCREEN_HEIGHT;
    _bus_width = buswidth;
}

void RA8889_common::setBusWidth(uint16_t buswidth) {
...
//**************************************************************//
boolean RA8889_common::RA8889Initialize() {
	...
	if(_bus_width == 16) {
		lcdDataWrite(RA8889_PLL_ENABLE<<7|RA8889_WAIT_NO_MASK<<6|RA8889_KEY_SCAN_DISABLE<<5|RA8889_TFT_OUTPUT24<<3
		|RA8889_I2C_MASTER_DISABLE<<2|RA8889_SERIAL_IF_ENABLE<<1|RA8889_HOST_DATA_BUS_16BIT);
	} else {
		lcdDataWrite(RA8889_PLL_ENABLE<<7|RA8889_WAIT_NO_MASK<<6|RA8889_KEY_SCAN_DISABLE<<5|RA8889_TFT_OUTPUT24<<3
		|RA8889_I2C_MASTER_DISABLE<<2|RA8889_SERIAL_IF_ENABLE<<1|RA8889_HOST_DATA_BUS_8BIT);
	}
#endif
    ...
    lcdDataWrite(RA8877_LVDS_FORMAT << 3 | RA8889_GRAPHIC_MODE << 2 | RA8889_MEMORY_SELECT_IMAGE);
...
//**********************************************************************
// Do a software reset of RA8889
//**********************************************************************
void RA8889_common::RA8889_SW_Reset(void) {
    unsigned char temp;
...
//**************************************************************//
void RA8889_common::checkWriteFifoNotFull(void) {
    ru16 i;
...
//**************************************************************//
void RA8889_common::checkWriteFifoEmpty(void) {
    ru16 i;
    for (i = 0; i < 10000; i++) // Please according to your usage to modify i value.
    {
...
//**************************************************************//
void RA8889_common::checkReadFifoNotFull(void) {
    ru16 i;
    for (i = 0; i < 10000; i++) // Please according to your usage to modify i value.
    {
...
//**************************************************************//
void RA8889_common::checkReadFifoNotEmpty(void) {
    ru16 i;
    for (i = 0; i < 10000; i++) // //Please according to your usage to modify i value.
    {
...
*****************************************************************/
void RA8889_common::check2dBusy(void) {
    ru32 i;
...
//**************************************************************//
boolean RA8889_common::checkSdramReady(void) {
    ru32 i;
    for (i = 0; i < 1000000; i++) // Please according to your usage to modify i value.
    {
        delayMicroseconds(1);
...
//**************************************************************//
boolean RA8889_common::checkIcReady(void) {
    ru32 i;
    for (i = 0; i < 1000000; i++) { // Please according to your usage to modify i value.
        delayMicroseconds(1);
...
//------------------------------------//----------------------------------*/
boolean RA8889_common::RA8889PllInitial(void) {
...
// Initialize SDRAM
//**************************************************************//
boolean RA8889_common::RA8889SdramInitial(void) {
    ru8 CAS_Latency;
    ru16 Auto_Refresh;
...
//**************************************************************//
void RA8889_common::displayOn(boolean on) {
    unsigned char temp;

    // Maybe preserve some of the bits.
    temp = lcdRegDataRead(RA8889_DPCR) & (1 << 3);

    if (on)
        lcdRegDataWrite(RA8889_DPCR, XPCLK_INV << 7 | RA8889_DISPLAY_ON << 6 | RA8889_OUTPUT_RGB | temp);
    else
        lcdRegDataWrite(RA8889_DPCR, XPCLK_INV << 7 | RA8889_DISPLAY_OFF << 6 | RA8889_OUTPUT_RGB | temp);
    delay(20);
}
...
//**************************************************************//
void RA8889_common::backlight(boolean on) {
    if (on) {
        ...
        // Select_PWM0();
        pwm_ClockMuxReg(0, RA8889_PWM_TIMER_DIV2, 0, RA8889_XPWM0_OUTPUT_PWM_TIMER0);
        // Enable_PWM0_Dead_Zone();
        // Auto_Reload_PWM0();
        // Start_PWM0();
        pwm_Configuration(RA8889_PWM_TIMER1_INVERTER_OFF, RA8889_PWM_TIMER1_AUTO_RELOAD, RA8889_PWM_TIMER1_STOP,
                          RA8889_PWM_TIMER0_DEAD_ZONE_ENABLE, RA8889_PWM_TIMER1_INVERTER_OFF,
                          RA8889_PWM_TIMER0_AUTO_RELOAD, RA8889_PWM_TIMER0_START);
        ...
        pwm_Configuration(RA8889_PWM_TIMER1_INVERTER_OFF, RA8889_PWM_TIMER1_AUTO_RELOAD, RA8889_PWM_TIMER1_STOP,
                          RA8889_PWM_TIMER0_DEAD_ZONE_ENABLE, RA8889_PWM_TIMER1_INVERTER_OFF,
                          RA8889_PWM_TIMER0_AUTO_RELOAD, RA8889_PWM_TIMER0_STOP);
    }
}

//**************************************************************//
//**************************************************************//
void RA8889_common::lcdHorizontalWidthVerticalHeight(ru16 width, ru16 height) {
    unsigned char temp;
    temp = (width / 8) - 1;
...
//**************************************************************//
//**************************************************************//
void RA8889_common::lcdHorizontalNonDisplay(ru16 numbers) {
    ru8 temp;
    if (numbers < 8) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::lcdHsyncStartPosition(ru16 numbers) {
    ru8 temp;
    if (numbers < 8) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::lcdHsyncPulseWidth(ru16 numbers) {
    ru8 temp;
    if (numbers < 8) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::lcdVerticalNonDisplay(ru16 numbers) {
    ru8 temp;
    temp = numbers - 1;
...
//**************************************************************//
//**************************************************************//
void RA8889_common::lcdVsyncStartPosition(ru16 numbers) {
    ru8 temp;
    temp = numbers - 1;
...
//**************************************************************//
//**************************************************************//
void RA8889_common::lcdVsyncPulseWidth(ru16 numbers) {
    ru8 temp;
    temp = numbers - 1;
...
//**************************************************************//
//**************************************************************//
void RA8889_common::displayImageStartAddress(ru32 addr) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::displayImageWidth(ru16 width) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::displayWindowStartXY(ru16 x0, ru16 y0) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::canvasImageStartAddress(ru32 addr) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::canvasImageWidth(ru16 width) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::activeWindowXY(ru16 x0, ru16 y0) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::activeWindowWH(ru16 width, ru16 height) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::setPixelCursor(ru16 x, ru16 y) {
    switch (_rotation) {
    case 1:
        swapvals(x, y);
        break;
...
//**************************************************************//
//**************************************************************//
void RA8889_common::linearAddressSet(ru32 addr) {
    lcdRegDataWrite(RA8889_CURH0, addr);       // 5fh
    lcdRegDataWrite(RA8889_CURH1, addr >> 8);  // 60h
    lcdRegDataWrite(RA8889_CURV0, addr >> 16); // 61h
    lcdRegDataWrite(RA8889_CURV1, addr >> 24); // 62h
}

//**************************************************************//
//**************************************************************//
ru8 RA8889_common::vmemReadData(ru32 addr) {
    ru8 vmemData = 0;
...
//**************************************************************//
//**************************************************************//
void RA8889_common::vmemWriteData(ru32 addr, ru8 vmemData) {
    graphicMode(true);
    Memory_Linear_Mode();
    linearAddressSet(addr);
    ramAccessPrepare();
...
//**************************************************************//
//**************************************************************//
ru16 RA8889_common::vmemReadData16(ru32 addr) {
    ru16 vmemData = 0;
...
//**************************************************************//
//**************************************************************//
void RA8889_common::vmemWriteData16(ru32 addr, ru16 vmemData) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_Source0_MemoryStartAddr(ru32 addr) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_Source0_ImageWidth(ru16 width) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_Source0_WindowStartXY(ru16 x0, ru16 y0) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_Source1_MemoryStartAddr(ru32 addr) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_Source1_ImageWidth(ru16 width) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_Source1_WindowStartXY(ru16 x0, ru16 y0) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_DestinationMemoryStartAddr(ru32 addr) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_DestinationImageWidth(ru16 width) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_DestinationWindowStartXY(ru16 x0, ru16 y0) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bte_WindowSize(ru16 width, ru16 height) {
...
//**************************************************************//
void RA8889_common::bte_WindowAlpha(ru8 alpha) {
    lcdRegDataWrite(RA8889_APB_CTRL, alpha); // b5h
}
...
//**************************************************************//
void RA8889_common::pwm_Prescaler(ru8 prescaler) {
    lcdRegDataWrite(RA8889_PSCLR, prescaler); // 84h
}

//**************************************************************//
//**************************************************************//
void RA8889_common::pwm_ClockMuxReg(ru8 pwm1_clk_div, ru8 pwm0_clk_div, ru8 xpwm1_ctrl, ru8 xpwm0_ctrl) {
    lcdRegDataWrite(RA8889_PMUXR, pwm1_clk_div << 6 | pwm0_clk_div << 4 | xpwm1_ctrl << 2 | xpwm0_ctrl); // 85h
}

//**************************************************************//
//**************************************************************//
void RA8889_common::pwm_Configuration(ru8 pwm1_inverter, ru8 pwm1_auto_reload, ru8 pwm1_start, ru8 pwm0_dead_zone, ru8 pwm0_inverter, ru8 pwm0_auto_reload, ru8 pwm0_start) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::pwm0_Duty(ru16 duty) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::pwm0_ClocksPerPeriod(ru16 clocks_per_period) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::pwm1_Duty(ru16 duty) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::pwm1_ClocksPerPeriod(ru16 clocks_per_period) {
...
//**************************************************************//
void RA8889_common::ramAccessPrepare(void) {
    lcdRegWrite(RA8889_MRWDP); // 04h
}

//**************************************************************//
//**************************************************************//
void RA8889_common::foreGroundColor16bpp(ru16 color, bool finalize) {
    lcdRegDataWrite(RA8889_FGCR, color >> 8, false);    // d2h
    lcdRegDataWrite(RA8889_FGCG, color >> 3, false);    // d3h
    lcdRegDataWrite(RA8889_FGCB, color << 3, finalize); // d4h
}

//**************************************************************//
//**************************************************************//
void RA8889_common::backGroundColor16bpp(ru16 color, bool finalize) {
    lcdRegDataWrite(RA8889_BGCR, color >> 8, false);    // d5h
    lcdRegDataWrite(RA8889_BGCG, color >> 3, false);    // d6h
    lcdRegDataWrite(RA8889_BGCB, color << 3, finalize); // d7h
}
...
//***************************************************//
//**************************************************************//
/* Turn RA8889 graphic mode ON/OFF (True = ON)                  */
/* Inverse of text mode                                         */
//**************************************************************//
void RA8889_common::graphicMode(boolean on) {
    if (on)
        lcdRegDataWrite(RA8889_ICR, RA8877_LVDS_FORMAT << 3 | RA8889_GRAPHIC_MODE << 2 | RA8889_MEMORY_SELECT_IMAGE); // 03h  //switch to graphic mode
    else
        lcdRegDataWrite(RA8889_ICR, RA8877_LVDS_FORMAT << 3 | RA8889_TEXT_MODE << 2 | RA8889_MEMORY_SELECT_IMAGE); // 03h  //switch back to text mode
}

//**************************************************************//
/* Read a 16bpp pixel                                           */
//**************************************************************//
ru16 RA8889_common::readPixel(int16_t x, int16_t y) {
    return getPixel(x, y);
}

ru16 RA8889_common::getPixel(ru16 x, ru16 y) {
    ru16 rdata = 0;
...
/* Write a 16bpp pixel                                          */
//**************************************************************//
void RA8889_common::drawPixel(ru16 x, ru16 y, ru16 color) {
    graphicMode(true);
    setPixelCursor(x, y);
    ramAccessPrepare();
	if(_bus_width == 16) {
...
void RA8889_common::readRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t *pcolors) {
...
//**************************************************************//
/* Turn on RA8889 Text Mode, (Same Graphics Mode in reverse)    */
//**************************************************************//
void RA8889_common::textMode(boolean on) {
    if (on)
        lcdRegDataWrite(RA8889_ICR, RA8877_LVDS_FORMAT << 3 | RA8889_TEXT_MODE << 2 | RA8889_MEMORY_SELECT_IMAGE); // 03h  //switch to text mode
    else
        lcdRegDataWrite(RA8889_ICR, RA8877_LVDS_FORMAT << 3 | RA8889_GRAPHIC_MODE << 2 | RA8889_MEMORY_SELECT_IMAGE); // 03h  //switch back to graphic mode
}

//**************************************************************//
/* Set text Foreground and Bckground colors (16 bit)            */
//**************************************************************//
void RA8889_common::textColor(ru16 foreground_color, ru16 background_color) {
    check2dBusy();
...
//**************************************************************//
void RA8889_common::setTextCursor(ru16 x, ru16 y) {
    lcdRegDataWrite(RA8889_F_CURX0, x, false);      // 63h
    lcdRegDataWrite(RA8889_F_CURX1, x >> 8, false); // 64h
    lcdRegDataWrite(RA8889_F_CURY0, y, false);      // 65h
    lcdRegDataWrite(RA8889_F_CURY1, y >> 8, true);  // 66h
    _cursorX = x;
    _cursorY = y; // Update global cursor position variables
}

//***************************************************************//
/* Position text cursor in character units                       */
//***************************************************************//
void RA8889_common::textxy(ru16 x, ru16 y) {
...
//**************************************************************//
void RA8889_common::setTextParameter1(ru8 source_select, ru8 size_select, ru8 iso_select) {
...
//***********************************************************************************************//
void RA8889_common::setTextParameter2(ru8 align, ru8 chroma_key, ru8 width_enlarge, ru8 height_enlarge) {
...
//**************************************************************//
void RA8889_common::genitopCharacterRomParameter(ru8 scs_select, ru8 clk_div, ru8 rom_select, ru8 character_select, ru8 gt_width) {
    if (scs_select == 0)
        lcdRegDataWrite(RA8889_SFL_CTRL, RA8889_SERIAL_FLASH_SELECT0 << 7 | RA8889_SERIAL_FLASH_FONT_MODE << 6 | RA8889_SERIAL_FLASH_ADDR_24BIT << 5 | RA8889_FOLLOW_RA8889_MODE << 4 | RA8889_SPI_FAST_READ_8DUMMY); // b7h
    if (scs_select == 1)
        lcdRegDataWrite(RA8889_SFL_CTRL, RA8889_SERIAL_FLASH_SELECT1 << 7 | RA8889_SERIAL_FLASH_FONT_MODE << 6 | RA8889_SERIAL_FLASH_ADDR_24BIT << 5 | RA8889_FOLLOW_RA8889_MODE << 4 | RA8889_SPI_FAST_READ_8DUMMY); // b7h
...
//**************************************************************//
void RA8889_common::clearStatusLine(uint16_t color) {
...
//**************************************************************//
void RA8889_common::writeStatusLine(ru16 x0, uint16_t fgcolor, uint16_t bgcolor, const char *str) {
...
void RA8889_common::clreol(void) {
    uint16_t tempX = _cursorX;
    uint16_t tempY = _cursorY;
...
void RA8889_common::clreos(void) {
    uint16_t tempX = _cursorX;
    uint16_t tempY = _cursorY;
    clreol();
...
// Support function for VT100: Clear to End Of Line
void RA8889_common::clrbol(void) {
    uint16_t tempX = _cursorX;
    uint16_t tempY = _cursorY;
...
void RA8889_common::clrbos(void) {
    uint16_t tempX = _cursorX;
    uint16_t tempY = _cursorY;
    clrbol();
...
void RA8889_common::clrlin(void) {
    uint16_t tempX = _cursorX;
    uint16_t tempY = _cursorY;
...
void RA8889_common::clearActiveScreen(void) {
...
//**************************************************************//
void RA8889_common::update_xy(void) {
...
//**************************************************************//
void RA8889_common::update_tft(uint8_t data) {
    CGRAM_Start_address(PATTERN1_RAM_START_ADDR);
    textMode(true);
    setTextCursor(_cursorX, _cursorY);
    ramAccessPrepare();
...
//********************************************************************//
void RA8889_common::buildTextScreen(void) {
    switch (_FNTheight) {
    case 16:
        _fontheight = 0;
        break;
    ...
    if (!UDFont) {
        setTextParameter1(RA8889_SELECT_INTERNAL_CGROM, _fontheight, RA8889_SELECT_8859_1); // cch
        setTextParameter2(RA8889_TEXT_FULL_ALIGN_ENABLE, RA8889_TEXT_CHROMA_KEY_DISABLE,
                          RA8889_TEXT_WIDTH_ENLARGEMENT_X1, RA8889_TEXT_HEIGHT_ENLARGEMENT_X1); // cdh
    } else {
        setTextParameter1(RA8889_SELECT_USER_DEFINED, RA8889_CHAR_HEIGHT_16, RA8889_SELECT_8859_1); // cch
        setTextParameter2(RA8889_TEXT_FULL_ALIGN_DISABLE, RA8889_TEXT_CHROMA_KEY_DISABLE,
                          _scaleX - 1, _scaleY - 1); // cdh
    }
    genitopCharacterRomParameter(RA8889_SERIAL_FLASH_SELECT0, RA8889_SPI_DIV4, RA8889_GT30L32S4W,
                                 RA8889_ASCII, RA8889_GT_FIXED_WIDTH);
...
/* RA8889 registers[DBh]~[DEh]                                        */
//********************************************************************//
void RA8889_common::CGRAM_Start_address(uint32_t Addr) {

    // CGRAM START ADDRESS [31:0]
    lcdRegDataWrite(RA8889_CGRAM_STR0, Addr);
    lcdRegDataWrite(RA8889_CGRAM_STR1, Addr >> 8);
    lcdRegDataWrite(RA8889_CGRAM_STR2, Addr >> 16);
    lcdRegDataWrite(RA8889_CGRAM_STR3, Addr >> 24);
}

//********************************************************************//
/* Initialize CGRAM with user defined font                            */
//********************************************************************//
void RA8889_common::CGRAM_initial(uint32_t charAddr, const uint8_t *data, uint16_t count) {
    uint16_t i;
    uint16_t tmp = 0;
    ...
    lcdRegWrite(RA8889_AW_COLOR);                                                  // 5Eh
    lcdDataWrite(RA8889_CANVAS_LINEAR_MODE << 2 | RA8889_CANVAS_COLOR_DEPTH_8BPP); // set memory to 8bpp and linear mode
...
//***************************************************************************//
void RA8889_common::Graphic_cursor_initial(void) {
    unsigned int i;
...
//***************************************************************************//
void RA8889_common::Upload_Graphic_Cursor(uint8_t cursorNum, uint8_t *data) {
    unsigned int i;
...
//***************************************************//
void RA8889_common::Memory_Select_CGRAM(void) { // this may not be right
    unsigned char temp;
    temp = lcdRegDataRead(RA8889_ICR); // 0x03
    //    temp &= cClrb1; // manual says this bit is clear
    temp |= cSetb0;
    temp &= cClrb0; // manual says this bit is set ?? Figure 14-10
    lcdRegDataWrite(RA8889_ICR, temp);
}

//***************************************************//
/* Select display buffer RAM                         */
//***************************************************//
void RA8889_common::Memory_Select_SDRAM(void) {
    unsigned char temp;
    temp = lcdRegDataRead(RA8889_ICR); // 0x03
    temp &= cClrb1;
    temp &= cClrb0; // B
...
//***************************************************//
void RA8889_common::Memory_Select_Graphic_Cursor_RAM(void) {
    unsigned char temp;
    temp = lcdRegDataRead(RA8889_ICR); // 0x03
    temp |= cSetb1;
    temp &= cClrb0;
...
//***************************************************//
void RA8889_common::Memory_XY_Mode(void) {
    /*
    ...
    */
    ...
    temp = lcdRegDataRead(RA8889_AW_COLOR);
    temp &= cClrb2;
    // lcdRegWrite(RA8889_AW_COLOR);
    lcdRegDataWrite(RA8889_AW_COLOR, temp);
}

//***************************************************//
/* Select RAM linear addressing mode                 */
//***************************************************//
void RA8889_common::Memory_Linear_Mode(void) {
    /*
    Canvas addressing mode
      0: Block mode (X-Y coordination addressing)
      1: linear mode
...
//***************************************************//
void RA8889_common::Enable_Graphic_Cursor(void) {
    /*
    ...
    */
    ...
    temp = lcdRegDataRead(RA8889_GTCCR); // 0x3c
    temp |= cSetb4;
    lcdRegDataWrite(RA8889_GTCCR, temp);
}

//***************************************************//
/* Disable Graphic Cursor                            */
//***************************************************//
void RA8889_common::Disable_Graphic_Cursor(void) {
    /*
    Graphic Cursor Enable
      0 : Graphic Cursor disable.
      1 : Graphic Cursor enable.
...
//************************************************/
void RA8889_common::Select_Graphic_Cursor_1(void) {
    /*
...
/* Select one of four available graphic cursors  */
//************************************************/
void RA8889_common::Select_Graphic_Cursor_2(void) {
    /*
    Graphic Cursor Selection Bit
    Select one from four graphic cursor types. (00b to 11b)
      00b : Graphic Cursor Set 1.
...
/* Select one of four available graphic cursors  */
//************************************************/
void RA8889_common::Select_Graphic_Cursor_3(void) {
    /*
    Graphic Cursor Selection Bit
    Select one from four graphic cursor types. (00b to 11b)
      00b : Graphic Cursor Set 1.
...
/* Select one of four available graphic cursors  */
//************************************************/
void RA8889_common::Select_Graphic_Cursor_4(void) {
    /*
    Graphic Cursor Selection Bit
    Select one from four graphic cursor types. (00b to 11b)
      00b : Graphic Cursor Set 1.
...
//************************************************/
void RA8889_common::Graphic_Cursor_XY(int16_t WX, int16_t HY) {
    /*
    ...
    */
    ...
    if (WX < 0 && WX > -32)
        WX = 0; // cursor partially visible off the left/top of the screen is coerced onto the screen
    if (HY < 0 && HY > -32)
        HY = 0;

    lcdRegDataWrite(RA8889_GCHP0, WX, false);
    lcdRegDataWrite(RA8889_GCHP1, WX >> 8, false);

    lcdRegDataWrite(RA8889_GCVP0, HY, false);
    lcdRegDataWrite(RA8889_GCVP1, HY >> 8, true);
}

//************************************************/
/* Set graphic cursor foreground color           */
//************************************************/
void RA8889_common::Set_Graphic_Cursor_Color_1(unsigned char temp) {
    /*
    REG[44h] Graphic Cursor Color 0 with 256 Colors
    RGB Format [7:0] = RRRGGGBB.
    */
    lcdRegDataWrite(RA8889_GCC0, temp);
}

//************************************************/
/* Set graphic cursor outline color              */
//************************************************/
void RA8889_common::Set_Graphic_Cursor_Color_2(unsigned char temp) {
    /*
    REG[45h] Graphic Cursor Color 1 with 256 Colors
    RGB Format [7:0] = RRRGGGBB.
    */
    lcdRegDataWrite(RA8889_GCC1, temp);
}

//************************************************/
/* turn on text cursor                           */
//************************************************/
void RA8889_common::Enable_Text_Cursor(void) {
    /*
    Text Cursor Enable
      0 : Disable.
      1 : Enable.
...
//************************************************/
void RA8889_common::Disable_Text_Cursor(void) {
    /*
    Text Cursor Enable
      0 : Disable.
      1 : Enable.
...
//************************************************/
void RA8889_common::Enable_Text_Cursor_Blinking(void) {
    /*
...
//************************************************/
void RA8889_common::Disable_Text_Cursor_Blinking(void) {
    /*
    Text Cursor Blinking Enable
      0 : Disable.
      1 : Enable.
...
//************************************************/
void RA8889_common::Blinking_Time_Frames(unsigned char temp) {
    /*
    Text Cursor Blink Time Setting (Unit: Frame)
      ...
      FFh : 256 frames time.
    */
    lcdRegDataWrite(RA8889_BTCR, temp); // 0x3d
}

//************************************************/
/* Set text cursor horizontal and vertical size  */
//************************************************/
void RA8889_common::Text_Cursor_H_V(unsigned short WX, unsigned short HY) {
    /*
    REG[3Eh]
    ...
    same times as the font enlargement.
    */
    lcdRegDataWrite(RA8889_CURHS, WX, false); // 0x3e
    lcdRegDataWrite(RA8889_CURVS, HY, true);  // 0x3f
}

//************************************************/
/* Turn on color bar test screen                 */
//************************************************/
void RA8889_common::Color_Bar_ON(void) {
    /*
    ...
    */
    unsigned char temp;
    temp = lcdRegDataRead(RA8889_DPCR); // 0x12
    temp |= cSetb5;
    lcdRegDataWrite(RA8889_DPCR, temp); // 0x12
}

//************************************************/
/* Turn off color bar test screen                */
//************************************************/
void RA8889_common::Color_Bar_OFF(void) {
    /*
    Display Test Color Bar
      0b: Disable.
      1b: Enable.
...
//**************************************************************//
void RA8889_common::drawLine(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 color) {
    x0 += _originx;
    x1 += _originx;
    y0 += _originy;
    y1 += _originy;
    ...
    lcdRegDataWrite(RA8889_DLHSR0, x0, false);            // 68h
    lcdRegDataWrite(RA8889_DLHSR1, x0 >> 8, false);       // 69h
    lcdRegDataWrite(RA8889_DLVSR0, y0, false);            // 6ah
    lcdRegDataWrite(RA8889_DLVSR1, y0 >> 8, false);       // 6bh
    lcdRegDataWrite(RA8889_DLHER0, x1, false);            // 6ch
    lcdRegDataWrite(RA8889_DLHER1, x1 >> 8, false);       // 6dh
    lcdRegDataWrite(RA8889_DLVER0, y1, false);            // 6eh
    lcdRegDataWrite(RA8889_DLVER1, y1 >> 8, false);       // 6fh
    lcdRegDataWrite(RA8889_DCR0, RA8889_DRAW_LINE, true); // 67h,0x80
}
...
//**************************************************************//
void RA8889_common::drawSquare(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 color) {
    x0 += _originx;
    x1 += _originx;
    y1 += _originy;
    y1 += _originy;
    ...
    if (x0 < _displayclipx1)
        x0 = _displayclipx1;
    if (y0 < _displayclipy1)
        y0 = _displayclipy1;
    ...
    lcdRegDataWrite(RA8889_DLVSR0, y0, false);              // 6ah
    lcdRegDataWrite(RA8889_DLVSR1, y0 >> 8, false);         // 6bh
    lcdRegDataWrite(RA8889_DLHER0, x_end, false);           // 6ch
    lcdRegDataWrite(RA8889_DLHER1, x_end >> 8, false);      // 6dh
    lcdRegDataWrite(RA8889_DLVER0, y1, false);              // 6eh
    lcdRegDataWrite(RA8889_DLVER1, y_end >> 8, false);      // 6fh
    lcdRegDataWrite(RA8889_DCR1, RA8889_DRAW_SQUARE, true); // 76h,0xa0
}
...
// x1,y1 is lower right end corner
//**************************************************************//
void RA8889_common::drawSquareFill(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 color) {
    ...
    lcdRegDataWrite(RA8889_DLHER0, x_end, false);                // 6ch
    lcdRegDataWrite(RA8889_DLHER1, x_end >> 8, false);           // 6dh
    lcdRegDataWrite(RA8889_DLVER0, y_end, false);                // 6eh
    lcdRegDataWrite(RA8889_DLVER1, y_end >> 8, false);           // 6fh
    lcdRegDataWrite(RA8889_DCR1, RA8889_DRAW_SQUARE_FILL, true); // 76h,0xE0
}
...
//**************************************************************//
void RA8889_common::drawCircleSquare(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 xr, ru16 yr, ru16 color) {
    x0 += _originx;
    x1 += _originx;
    y1 += _originy;
    y1 += _originy;
    ...
    lcdRegDataWrite(RA8889_DLVER0, y_end, false);                  // 6eh
    lcdRegDataWrite(RA8889_DLVER1, y_end >> 8, false);             // 6fh
    lcdRegDataWrite(RA8889_ELL_A0, xr, false);                     // 77h
    lcdRegDataWrite(RA8889_ELL_A1, xr >> 8, false);                // 79h
    lcdRegDataWrite(RA8889_ELL_B0, yr, false);                     // 7ah
    lcdRegDataWrite(RA8889_ELL_B1, yr >> 8, false);                // 7bh
    lcdRegDataWrite(RA8889_DCR1, RA8889_DRAW_CIRCLE_SQUARE, true); // 76h,0xb0
}
...
// yr is the minor radius of corner (vertical)
//**************************************************************//
void RA8889_common::drawCircleSquareFill(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 xr, ru16 yr, ru16 color) {
    x0 += _originx;
    x1 += _originx;
    y1 += _originy;
    y1 += _originy;
    ...
    lcdRegDataWrite(RA8889_DLVER1, y_end >> 8, false);                  // 6fh
    lcdRegDataWrite(RA8889_ELL_A0, xr, false);                          // 77h
    lcdRegDataWrite(RA8889_ELL_A1, xr >> 8, false);                     // 78h
    lcdRegDataWrite(RA8889_ELL_B0, yr, false);                          // 79h
    lcdRegDataWrite(RA8889_ELL_B1, yr >> 8, false);                     // 7ah
    lcdRegDataWrite(RA8889_DCR1, RA8889_DRAW_CIRCLE_SQUARE_FILL, true); // 76h,0xf0
}
...
//**************************************************************//
void RA8889_common::drawTriangle(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 x2, ru16 y2, ru16 color) {
    x0 += _originx;
    x1 += _originx;
    x2 += _originx;
    y0 += _originy;
    ...
    graphicMode(true);
    foreGroundColor16bpp(color);
    lcdRegDataWrite(RA8889_DLHSR0, x0, false);                 // 68h point 0
    lcdRegDataWrite(RA8889_DLHSR1, x0 >> 8, false);            // 69h point 0
    lcdRegDataWrite(RA8889_DLVSR0, y0, false);                 // 6ah point 0
    lcdRegDataWrite(RA8889_DLVSR1, y0 >> 8, false);            // 6bh point 0
    lcdRegDataWrite(RA8889_DLHER0, x1, false);                 // 6ch point 1
    lcdRegDataWrite(RA8889_DLHER1, x1 >> 8, false);            // 6dh point 1
    lcdRegDataWrite(RA8889_DLVER0, y1, false);                 // 6eh point 1
    lcdRegDataWrite(RA8889_DLVER1, y1 >> 8, false);            // 6fh point 1
    lcdRegDataWrite(RA8889_DTPH0, x2, false);                  // 70h point 2
    lcdRegDataWrite(RA8889_DTPH1, x2 >> 8, false);             // 71h point 2
    lcdRegDataWrite(RA8889_DTPV0, y2, false);                  // 72h point 2
    lcdRegDataWrite(RA8889_DTPV1, y2 >> 8, false);             // 73h  point 2
    lcdRegDataWrite(RA8889_DCR0, RA8889_DRAW_TRIANGLE, false); // 67h,0x82
}
...
// x2,y2 is triangle end point
//**************************************************************//
void RA8889_common::drawTriangleFill(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 x2, ru16 y2, ru16 color) {
    x0 += _originx;
    x1 += _originx;
    x2 += _originx;
    y0 += _originy;
    ...
    lcdRegDataWrite(RA8889_DLVER0, y1, false);                     // 6eh
    lcdRegDataWrite(RA8889_DLVER1, y1 >> 8, false);                // 6fh
    lcdRegDataWrite(RA8889_DTPH0, x2, false);                      // 70h
    lcdRegDataWrite(RA8889_DTPH1, x2 >> 8, false);                 // 71h
    lcdRegDataWrite(RA8889_DTPV0, y2, false);                      // 72h
    lcdRegDataWrite(RA8889_DTPV1, y2 >> 8, false);                 // 73h
    lcdRegDataWrite(RA8889_DCR0, RA8889_DRAW_TRIANGLE_FILL, true); // 67h,0xa2
}
...
// See page 59 of RA8889.pdf for information on drawing arc's. (1 of 4 quadrants at a time only)
//**************************************************************//
void RA8889_common::drawCircle(ru16 x0, ru16 y0, ru16 r, ru16 color) {
    x0 += _originx;
    y0 += _originy;
    ...
    if (r > _height / 2)
        r = (_height / 2) - 1; // this is the (undocumented) hardware limit of RA8875
    check2dBusy();
    graphicMode(true);
    foreGroundColor16bpp(color);
    switch (_rotation) {
    ...
    lcdRegDataWrite(RA8889_DEHR0, x0, false);      // 7bh
    lcdRegDataWrite(RA8889_DEHR1, x0 >> 8, false); // 7ch
    lcdRegDataWrite(RA8889_DEVR0, y0, false);      // 7dh
    lcdRegDataWrite(RA8889_DEVR1, y0 >> 8, false); // 7eh
    lcdRegDataWrite(RA8889_ELL_A0, r, false);      // 77h
    lcdRegDataWrite(RA8889_ELL_A1, r >> 8, false); // 78h
    lcdRegDataWrite(RA8889_ELL_B0, r, false);      // 79h
    lcdRegDataWrite(RA8889_ELL_B1, r >> 8, false); // 7ah
    //  lcdRegDataWrite(RA8889_DCR1,RA8889_DRAW_BOTTOM_LEFT_CURVE);//76h,0x90 (arc test)
    lcdRegDataWrite(RA8889_DCR1, RA8889_DRAW_CIRCLE, true); // 76h,0x80
}
...
// See page 59 of RA8889.pdf for information on drawing arc's. (1 of 4 quadrants at a time only)
//**************************************************************//
void RA8889_common::drawCircleFill(ru16 x0, ru16 y0, ru16 r, ru16 color) {
    x0 += _originx;
    y0 += _originy;
    ...
    lcdRegDataWrite(RA8889_ELL_B0, r, false);      // 79h
    lcdRegDataWrite(RA8889_ELL_B1, r >> 8, false); // 7ah
    //  lcdRegDataWrite(RA8889_DCR1,RA8889_DRAW_BOTTOM_LEFT_CURVE_FILL);//76h,0xd0 (arc test)
    lcdRegDataWrite(RA8889_DCR1, RA8889_DRAW_CIRCLE_FILL, false); // 76h,0xc0
}
...
//**************************************************************//
void RA8889_common::drawEllipse(ru16 x0, ru16 y0, ru16 xr, ru16 yr, ru16 color) {
    x0 += _originx;
    y0 += _originy;
    ...
    lcdRegDataWrite(RA8889_ELL_B0, yr, false);               // 79h
    lcdRegDataWrite(RA8889_ELL_B1, yr >> 8, false);          // 7ah
    lcdRegDataWrite(RA8889_DCR1, RA8889_DRAW_ELLIPSE, true); // 76h,0x80
}
...
//**************************************************************//
void RA8889_common::drawEllipseFill(ru16 x0, ru16 y0, ru16 xr, ru16 yr, ru16 color) {
    x0 += _originx;
    y0 += _originy;
    ...
    lcdRegDataWrite(RA8889_ELL_B0, yr, false);                    // 79h
    lcdRegDataWrite(RA8889_ELL_B1, yr >> 8, false);               // 7ah
    lcdRegDataWrite(RA8889_DCR1, RA8889_DRAW_ELLIPSE_FILL, true); // 76h,0xc0
}

//*************************************************************//
// Scroll Screen up
//*************************************************************//
void RA8889_common::scroll(void) {                                                          // No arguments for now
...
// Scroll Screen down
//*************************************************************//
void RA8889_common::scrollDown(void) {                                                      // No arguments for now
...
//*************************************************************//
uint32_t RA8889_common::boxPut(uint32_t vPageAddr, uint16_t x0, uint16_t y0,
...
//*************************************************************//
uint32_t RA8889_common::boxGet(uint32_t vPageAddr, uint16_t x0, uint16_t y0,
                               uint16_t x1, uint16_t y1,
                               uint16_t dx0, uint16_t dy0) {
...
//**************************************************************//
void RA8889_common::bteMemoryCopy(ru32 s0_addr, ru16 s0_image_width, ru16 s0_x, ru16 s0_y,
    ...
    //  lcdRegDataWrite(RA8889_BTE_CTRL1,RA8889_BTE_ROP_CODE_12<<4|3);//91h
    lcdRegDataWrite(RA8889_BTE_COLR, (RA8889_S0_COLOR_DEPTH_16BPP << 5 | RA8889_S1_COLOR_DEPTH_16BPP << 2 | RA8889_DESTINATION_COLOR_DEPTH_16BPP) & 0x7f); // 92h
...
//**************************************************************//
void RA8889_common::bteMemoryCopyWithROP(
...
//**************************************************************//
void RA8889_common::bteMemoryCopyWithChromaKey(
    ru32 s0_addr, ru16 s0_image_width, ru16 s0_x, ru16 s0_y,
    ru32 des_addr, ru16 des_image_width, ru16 des_x, ru16 des_y,
...
//**************************************************************//
void RA8889_common::bteMemoryCopyWindowAlpha(
    ru32 s0_addr, ru16 s0_image_width, ru16 s0_x, ru16 s0_y,
    ru32 s1_addr, ru16 s1_image_width, ru16 s1_x, ru16 s1_y,
    ru32 des_addr, ru16 des_image_width, ru16 des_x, ru16 des_y,
...
//**************************************************************//
void RA8889_common::bteMpuWriteWithChromaKey(ru32 des_addr, ru16 des_image_width, ru16 des_x, ru16 des_y, ru16 width,
...
//**************************************************************//
//**************************************************************//
void RA8889_common::bteMpuWriteColorExpansionData(ru32 des_addr, ru16 des_image_width, ru16 des_x, ru16 des_y, ru16 width,
...
// write data after setting, using lcdDataWrite() or lcdDataWrite16bbp()
//**************************************************************//
void RA8889_common::bteMpuWriteColorExpansion(ru32 des_addr, ru16 des_image_width, ru16 des_x, ru16 des_y, ru16 width,
...
//**************************************************************//
void RA8889_common::bteMpuWriteColorExpansionWithChromaKeyData(ru32 des_addr, ru16 des_image_width, ru16 des_x, ru16 des_y, ru16 width,
                                                               ru16 height, ru16 foreground_color, ru16 background_color, const unsigned char *data) {
    ru16 i, j;
    check2dBusy();
    graphicMode(true);
...
// write data after setting, using lcdDataWrite() or lcdDataWrite16bbp()
//**************************************************************//
void RA8889_common::bteMpuWriteColorExpansionWithChromaKey(ru32 des_addr, ru16 des_image_width, ru16 des_x, ru16 des_y, ru16 width,
                                                           ru16 height, ru16 foreground_color, ru16 background_color) {
    check2dBusy();
    graphicMode(true);
    bte_DestinationMemoryStartAddr(des_addr);
...
//**************************************************************//
//**************************************************************//
void RA8889_common::btePatternFill(ru8 p8x8or16x16, ru32 s0_addr, ru16 s0_image_width, ru16 s0_x, ru16 s0_y,
    ...
    bte_DestinationWindowStartXY(des_x, des_y);
    bte_WindowSize(width, height);
    lcdRegDataWrite(RA8889_BTE_CTRL1, RA8889_BTE_ROP_CODE_12 << 4 | RA8889_BTE_PATTERN_FILL_WITH_ROP);                                            // 91h
    lcdRegDataWrite(RA8889_BTE_COLR, RA8889_S0_COLOR_DEPTH_16BPP << 5 | RA8889_S1_COLOR_DEPTH_16BPP << 2 | RA8889_DESTINATION_COLOR_DEPTH_16BPP); // 92h

    if (p8x8or16x16 == 0)
        lcdRegDataWrite(RA8889_BTE_CTRL0, RA8889_BTE_ENABLE << 4 | RA8889_PATTERN_FORMAT8X8); // 90h
    else
        lcdRegDataWrite(RA8889_BTE_CTRL0, RA8889_BTE_ENABLE << 4 | RA8889_PATTERN_FORMAT16X16); // 90h
}

//**************************************************************//
//**************************************************************//
void RA8889_common::btePatternFillWithChromaKey(ru8 p8x8or16x16, ru32 s0_addr, ru16 s0_image_width, ru16 s0_x, ru16 s0_y, ru32 des_addr,
    ...
    bte_WindowSize(width, height);
    backGroundColor16bpp(chromakey_color);
    lcdRegDataWrite(RA8889_BTE_CTRL1, RA8889_BTE_ROP_CODE_12 << 4 | RA8889_BTE_PATTERN_FILL_WITH_CHROMA);                                         // 91h
    lcdRegDataWrite(RA8889_BTE_COLR, RA8889_S0_COLOR_DEPTH_16BPP << 5 | RA8889_S1_COLOR_DEPTH_16BPP << 2 | RA8889_DESTINATION_COLOR_DEPTH_16BPP); // 92h
...
/*DMA Function*/
//**************************************************************//
/*If used 32bit address serial flash through RA8889, must be set command to serial flash to enter 4bytes mode first.
only needs set one times after power on */
//**************************************************************//
void RA8889_common::setSerialFlash4BytesMode(ru8 scs_select) {
    if (scs_select == 0) {
        lcdRegDataWrite(RA8889_SPIMCR2, RA8889_SPIM_NSS_SELECT_0 << 5 | RA8889_SPIM_MODE0);                               // b9h
        lcdRegDataWrite(RA8889_SPIMCR2, RA8889_SPIM_NSS_SELECT_0 << 5 | RA8889_SPIM_NSS_ACTIVE << 4 | RA8889_SPIM_MODE0); // b9h
        lcdRegWrite(RA8889_SPIDR);                                                                                        // b8h
        delay(1);
        lcdDataWrite(0xB7); //
        delay(1);
        lcdRegDataWrite(RA8889_SPIMCR2, RA8889_SPIM_NSS_SELECT_0 << 5 | RA8889_SPIM_NSS_INACTIVE << 4 | RA8889_SPIM_MODE0); // b9h
    }
    if (scs_select == 1) {
        lcdRegDataWrite(RA8889_SPIMCR2, RA8889_SPIM_NSS_SELECT_1 << 5 | RA8889_SPIM_MODE0);                               // b9h
        lcdRegDataWrite(RA8889_SPIMCR2, RA8889_SPIM_NSS_SELECT_1 << 5 | RA8889_SPIM_NSS_ACTIVE << 4 | RA8889_SPIM_MODE0); // b9h
        lcdRegWrite(RA8889_SPIDR);                                                                                        // b8h
        delay(1);
        lcdDataWrite(0xB7); //
        delay(1);
        lcdRegDataWrite(RA8889_SPIMCR2, RA8889_SPIM_NSS_SELECT_1 << 5 | RA8889_SPIM_NSS_INACTIVE << 4 | RA8889_SPIM_MODE0); // b9h
    }
}
...
//**************************************************************//
void RA8889_common::dma_24bitAddressBlockMode(ru8 scs_select, ru8 clk_div, ru16 x0, ru16 y0, ru16 width, ru16 height, ru16 picture_width, ru32 addr) {
    if (scs_select == 0)
        lcdRegDataWrite(RA8889_SFL_CTRL, RA8889_SERIAL_FLASH_SELECT0 << 7 | RA8889_SERIAL_FLASH_DMA_MODE << 6 | RA8889_SERIAL_FLASH_ADDR_24BIT << 5 | RA8889_FOLLOW_RA8889_MODE << 4 | RA8889_SPI_FAST_READ_8DUMMY); // b7h
    if (scs_select == 1)
        lcdRegDataWrite(RA8889_SFL_CTRL, RA8889_SERIAL_FLASH_SELECT1 << 7 | RA8889_SERIAL_FLASH_DMA_MODE << 6 | RA8889_SERIAL_FLASH_ADDR_24BIT << 5 | RA8889_FOLLOW_RA8889_MODE << 4 | RA8889_SPI_FAST_READ_8DUMMY); // b7h
...
/* scs = 0 : select scs0, scs = 1 : select scs1, */
//**************************************************************//
void RA8889_common::dma_32bitAddressBlockMode(ru8 scs_select, ru8 clk_div, ru16 x0, ru16 y0, ru16 width, ru16 height, ru16 picture_width, ru32 addr) {
    if (scs_select == 0)
        lcdRegDataWrite(RA8889_SFL_CTRL, RA8889_SERIAL_FLASH_SELECT0 << 7 | RA8889_SERIAL_FLASH_DMA_MODE << 6 | RA8889_SERIAL_FLASH_ADDR_32BIT << 5 | RA8889_FOLLOW_RA8889_MODE << 4 | RA8889_SPI_FAST_READ_8DUMMY); // b7h
    if (scs_select == 1)
        lcdRegDataWrite(RA8889_SFL_CTRL, RA8889_SERIAL_FLASH_SELECT1 << 7 | RA8889_SERIAL_FLASH_DMA_MODE << 6 | RA8889_SERIAL_FLASH_ADDR_32BIT << 5 | RA8889_FOLLOW_RA8889_MODE << 4 | RA8889_SPI_FAST_READ_8DUMMY); // b7h
...
//[2Ah][2Bh][2Ch][2Dh]=========================================================================
void RA8889_common::PIP_Display_Start_XY(unsigned short WX, unsigned short HY) {
    /*
    ...
    */
    lcdRegDataWrite(RA8889_PWDULX0, WX);      // [2Ah] PIP Window Display Upper-Left corner X-coordination [7:0]
    lcdRegDataWrite(RA8889_PWDULX1, WX >> 8); // [2Bh] PIP Window Display Upper-Left corner X-coordination [12:8]
                                              /*
                                              Reference Main Window coordination.
                                              Unit: Pixel
                                              ...
                                              Function bit will be configured for relative PIP window.
                                              */
    lcdRegDataWrite(RA8889_PWDULY0, HY);      // [2Ch] PIP Window Display Upper-Left corner Y-coordination [7:0]
    lcdRegDataWrite(RA8889_PWDULY1, HY >> 8); // [2Dh] PIP Window Display Upper-Left corner Y-coordination [12:8]
}

//[2Eh][2Fh][30h][31h]=========================================================================
void RA8889_common::PIP_Image_Start_Address(unsigned long Addr) {
    lcdRegDataWrite(RA8889_PISA0, Addr);       // [2Eh] PIP Image Start Address[7:2]
    lcdRegDataWrite(RA8889_PISA1, Addr >> 8);  // [2Fh] PIP Image Start Address[15:8]
    lcdRegDataWrite(RA8889_PISA2, Addr >> 16); // [30h] PIP Image Start Address [23:16]
    lcdRegDataWrite(RA8889_PISA3, Addr >> 24); // [31h] PIP Image Start Address [31:24]
}

//[32h][33h]=========================================================================
void RA8889_common::PIP_Image_Width(unsigned short WX) {
    /*
    Unit: Pixel.
    ...
    Function bit will be configured for relative PIP window.
    */
    lcdRegDataWrite(RA8889_PIW0, WX);      // [32h] PIP Image Width [7:0]
    lcdRegDataWrite(RA8889_PIW1, WX >> 8); // [33h] PIP Image Width [12:8]
}

//[34h][35h][36h][37h]=========================================================================
void RA8889_common::PIP_Window_Image_Start_XY(unsigned short WX, unsigned short HY) {
    /*
    ...
    Function bit will be configured for relative PIP window.
    */
    lcdRegDataWrite(RA8889_PWIULX0, WX);      // [34h] PIP 1 or 2 Window Image Upper-Left corner X-coordination [7:0]
    lcdRegDataWrite(RA8889_PWIULX1, WX >> 8); // [35h] PIP Window Image Upper-Left corner X-coordination [12:8]
                                              /*
                                              Reference PIP Image coordination.
                                              Unit: Pixel
                                              ...
                                              Function bit will be configured for relative PIP window.
                                              */
    lcdRegDataWrite(RA8889_PWIULY0, HY);      // [36h] PIP Windows Display Upper-Left corner Y-coordination [7:0]
    lcdRegDataWrite(RA8889_PWIULY1, HY >> 8); // [37h] PIP Windows Image Upper-Left corner Y-coordination [12:8]
}

//[38h][39h][3Ah][3Bh]=========================================================================
void RA8889_common::PIP_Window_Width_Height(unsigned short WX, unsigned short HY) {
    /*
    Unit: Pixel.
    ...
    Function bit will be configured for relative PIP window.
    */
    lcdRegDataWrite(RA8889_PWW0, WX);      // [38h] PIP Window Width [7:0]
    lcdRegDataWrite(RA8889_PWW1, WX >> 8); // [39h] PIP Window Width [10:8]
                                           /*
                                           Unit: Pixel
                                           ...
                                           Function bit will be configured for relative PIP window.
                                           */
    lcdRegDataWrite(RA8889_PWH0, HY);      // [3Ah] PIP Window Height [7:0]
    lcdRegDataWrite(RA8889_PWH1, HY >> 8); // [3Bh] PIP Window Height [10:8]
}

//[10h]=========================================================================
// Turn on PIP window 1
void RA8889_common::Enable_PIP1(void) {
    /*
    ...
    */
    unsigned char temp;
    temp = lcdRegDataRead(RA8889_MPWCTR); // 0x10
    temp |= cSetb7;
    lcdRegDataWrite(RA8889_MPWCTR, temp);
}

// Turn off PIP window 1
void RA8889_common::Disable_PIP1(void) {
    /*
    PIP 1 window Enable/Disable
    0 : PIP 1 window disable.
    1 : PIP 1 window enable
...
void RA8889_common::Enable_PIP2(void) {
    /*
...
// Turn off PIP window 1
void RA8889_common::Disable_PIP2(void) {
    /*
    PIP 2 window Enable/Disable
    0 : PIP 2 window disable.
    1 : PIP 2 window enable
...
void RA8889_common::Select_PIP1_Parameter(void) {
    /*
...
void RA8889_common::Select_PIP2_Parameter(void) {
    /*
    0: To configure PIP 1¡Šs parameters.
    1: To configure PIP 2¡Šs parameters..
    */
...
void RA8889_common::Select_Main_Window_8bpp(void) {
    /*
...
void RA8889_common::Select_Main_Window_16bpp(void) {
    /*
    Main Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
    01b: 16-bpp generic TFT, i.e. 65K colors.
...
void RA8889_common::Select_Main_Window_24bpp(void) {
    /*
    Main Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
    01b: 16-bpp generic TFT, i.e. 65K colors.
...
void RA8889_common::Select_LCD_Sync_Mode(void) {
    /*
...
void RA8889_common::Select_LCD_DE_Mode(void) {
    /*
    To Control panel's synchronous signals
    0: Sync Mode: Enable XVSYNC, XHSYNC, XDE
    1: DE Mode: Only XDE enable, XVSYNC & XHSYNC in idle state
...
//[11h]=========================================================================
void RA8889_common::Select_PIP1_Window_8bpp(void) {
    /*
    ...
    */
    unsigned char temp;
    temp = lcdRegDataRead(RA8889_PIPCDEP); // 0x11
    temp &= cClrb3;
    temp &= cClrb2;
    lcdRegDataWrite(RA8889_PIPCDEP, temp);
}

void RA8889_common::Select_PIP1_Window_16bpp(void) {
    /*
    PIP 1 Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
    01b: 16-bpp generic TFT, i.e. 65K colors.
...
void RA8889_common::Select_PIP1_Window_24bpp(void) {
    /*
    PIP 1 Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
    01b: 16-bpp generic TFT, i.e. 65K colors.
...
void RA8889_common::Select_PIP2_Window_8bpp(void) {
    /*
...
void RA8889_common::Select_PIP2_Window_16bpp(void) {
    /*
    PIP 2 Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
    01b: 16-bpp generic TFT, i.e. 65K colors.
...
void RA8889_common::Select_PIP2_Window_24bpp(void) {
    /*
    PIP 2 Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
    01b: 16-bpp generic TFT, i.e. 65K colors.
...
void RA8889_common::selectScreen(uint32_t screenPage) {
    check2dBusy();
...
// Save current screen page parameters
void RA8889_common::saveTFTParams(tftSave_t *screenSave) {
...
// Restore selected screen page parameters
void RA8889_common::restoreTFTParams(tftSave_t *screenSave) {
...
void RA8889_common::useCanvas(boolean on) {
    if (on) {
...
void RA8889_common::updateScreen() {
...
// Setup text cursor
void RA8889_common::cursorInit(void) {
    _cursorXsize = _FNTwidth;
...
// void RA8889_common::setCursor(uint16_t x, uint16_t y)
//{
//   setTextCursor(x, y);
// }
...
*/
/**************************************************************************/
void RA8889_common::setCursor(int16_t x, int16_t y, bool autocenter) {
    if (_use_default) {
        // setTextCursor(x, y);
        // return;
    }
    ...
    if (_relativeCenter || _absoluteCenter)
        return;
    if (bitRead(_TXTparameters, 7) == 0)
        _textPosition(x, y, false);
}
...
*/
/**************************************************************************/
void RA8889_common::getCursor(int16_t &x, int16_t &y) {
    uint8_t t1, t2, t3, t4;
    t1 = lcdRegDataRead(RA8889_F_CURX0);
    t2 = lcdRegDataRead(RA8889_F_CURX1);
    t3 = lcdRegDataRead(RA8889_F_CURY0);
    t4 = lcdRegDataRead(RA8889_F_CURY1);
    x = (t2 << 8) | (t1 & 0xFF);
    y = (t4 << 8) | (t3 & 0xFF);
    // if (_portrait && _use_default) swapvals(x,y);
}

int16_t RA8889_common::getCursorX(void) {
    // if (_portrait && _use_default) return _cursorY;
    return _cursorX;
}

int16_t RA8889_common::getCursorY(void) {
    // if (_portrait && _use_default) return _cursorX;
    return _cursorY;
}

// Set Graphic Mode Margins (pixel based)
void RA8889_common::setMargins(uint16_t xl, uint16_t yt, uint16_t xr, uint16_t yb) {
    _scrollXL = xl;
    _scrollYT = yt;
    _scrollXR = xr;
    _scrollYB = yb;
...
void RA8889_common::setTMargins(uint16_t xl, uint16_t yt, uint16_t xr, uint16_t yb) {
...
//    fillRect(_scrollXL, _scrollYT, _scrollXR, _scrollYB, color);
    textColor(_TXTForeColor, _TXTBackColor);
    setTextCursor(_scrollXL, _scrollYT);
//    check2dBusy(); // must wait for fill to finish before setting foreground color
}
...
void RA8889_common::fillStatusLine(uint16_t color) {
...
//**************************************************************//
void RA8889_common::setTextColor(uint16_t color) {
...
//**************************************************************//
//**************************************************************//
void RA8889_common::setBackGroundColor(uint16_t color) {
    backGroundColor16bpp(color);
}

// Set text foreground + background colors
void RA8889_common::setTextColor(uint16_t fgc, uint16_t bgc) {
    if (_use_ili_font) {
...
void RA8889_common::tftRawWrite(uint8_t data) {
    rawPrint(data);
}

// Send a string to the status line
void RA8889_common::printStatusLine(uint16_t x0, uint16_t fgColor, uint16_t bgColor, const char *text) {
...
// Load a user defined font from memory to RA8889 Character generator RAM
uint8_t RA8889_common::fontLoadMEM(char *fontsrc) {
...
void RA8889_common::setFontSource(uint8_t source) {
...
boolean RA8889_common::setFontSize(uint8_t scale, boolean runflag) {
    switch (scale) {
    ...
    case 0:
        if (UDFont) { // User Defined Fonts
			...
			// RA8889 has only 12x24 internal font size!!
            _FNTheight = 24;
            _FNTwidth = 12;
        }
        break;
...
void RA8889_common::setCursorMode(boolean mode) {
...
void RA8889_common::setCursorType(uint8_t type) {
...
void RA8889_common::setCursorBlink(boolean onOff) {
    if (onOff)
...
void RA8889_common::drawRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
    ...
    lcdRegDataWrite(RA8889_DLHSR0, x, false);               // 68h
    lcdRegDataWrite(RA8889_DLHSR1, x >> 8, false);          // 69h
    lcdRegDataWrite(RA8889_DLVSR0, y, false);               // 6ah
    lcdRegDataWrite(RA8889_DLVSR1, y >> 8, false);          // 6bh
    lcdRegDataWrite(RA8889_DLHER0, x_end, false);           // 6ch
    lcdRegDataWrite(RA8889_DLHER1, x_end >> 8, false);      // 6dh
    lcdRegDataWrite(RA8889_DLVER0, y_end, false);           // 6eh
    lcdRegDataWrite(RA8889_DLVER1, y_end >> 8, false);      // 6fh
...
void RA8889_common::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
    x += _originx;
    y += _originy;
    int16_t x_end = x + w - 1;
    int16_t y_end = y + h - 1;
...
void RA8889_common::fillRectHGradient(int16_t x, int16_t y, int16_t w, int16_t h,
...
void RA8889_common::fillRectVGradient(int16_t x, int16_t y, int16_t w, int16_t h,
                                      uint16_t color1, uint16_t color2) {
    x += _originx;
    y += _originy;
    int16_t x_end = x + w - 1;
...
void RA8889_common::writeRect(int16_t x, int16_t y, int16_t w, int16_t h, const uint16_t *pcolors) {
    x += _originx;
    y += _originy;
...
void RA8889_common::writeRectImpl(int16_t x, int16_t y, int16_t w, int16_t h, const uint16_t *pcolors) {
...
void RA8889_common::putPicture(ru16 x, ru16 y, ru16 w, ru16 h, const unsigned char *data) {
    ...
    //RA8889_Lite::putPicture_16bppData8(x, y, w, h, data);
    //Using the BTE function is faster and will use DMA if available
  if(_bus_width == 16) {
...
uint16_t *RA8889_common::rotateImageRect(int16_t w, int16_t h, const uint16_t *pcolors, int16_t rotation) {
    ...
    if ((rotation < 0) || (rotation > 3))
        rotation = _rotation; // just use current one.
...
void RA8889_common::writeRotatedRect(int16_t x, int16_t y, int16_t w, int16_t h, const uint16_t *pcolors) {
...
void RA8889_common::writeRect8BPP(int16_t x, int16_t y, int16_t w, int16_t h,
...
void RA8889_common::writeRect4BPP(int16_t x, int16_t y, int16_t w, int16_t h,
                                  const uint8_t *pixels,
                                  const uint16_t *palette) {
...
void RA8889_common::writeRect2BPP(int16_t x, int16_t y, int16_t w, int16_t h,
                                  const uint8_t *pixels,
                                  const uint16_t *palette) {
    // Simply call through our helper
...
void RA8889_common::writeRect1BPP(int16_t x, int16_t y, int16_t w, int16_t h,
                                  const uint8_t *pixels,
                                  const uint16_t *palette) {
    // Simply call through our helper
...
//  the work.
void RA8889_common::writeRectNBPP(int16_t x, int16_t y, int16_t w, int16_t h,
...
void RA8889_common::drawRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t xr, uint16_t yr, uint16_t color) {
    x += _originx;
    y += _originy;
    int16_t x_end = x + w - 1;
    int16_t y_end = y + h - 1;
...
void RA8889_common::fillRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t xr, uint16_t yr, uint16_t color) {
    x += _originx;
    y += _originy;
    int16_t x_end = x + w - 1;
    int16_t y_end = y + h - 1;
...
// Draw a filled circle
void RA8889_common::fillCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color) {
    drawCircleFill(x0, y0, r, color);
}

// Draw a filled ellipse.
void RA8889_common::fillEllipse(int16_t xCenter, int16_t yCenter, int16_t longAxis, int16_t shortAxis, uint16_t color) {
    drawEllipseFill(xCenter, yCenter, longAxis, shortAxis, color);
}

// Draw a filled triangle
void RA8889_common::fillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color) {
    drawTriangleFill(x0, y0, x1, y1, x2, y2, color);
}

// Setup graphic mouse cursor
void RA8889_common::gCursorSet(boolean gCursorEnable, uint8_t gcursortype, uint8_t gcursorcolor1, uint8_t gcursorcolor2) {
    if (!gCursorEnable)
...
void RA8889_common::gcursorxy(uint16_t gcx, uint16_t gcy) {
    Graphic_Cursor_XY(gcx, gcy);
}

// Scroll the screen up one text line
void RA8889_common::scrollUp(void) {
    scroll();
}

//=======================================================================
//= RA8889 BTE functions (Block Transfer Engine)                        =
//=======================================================================
// Copy box size part of the current screen page to another screen page
/*uint32_t RA8889_common::tft_boxPut(uint32_t vPageAddr, uint16_t x0, uint16_t y0,
...
uint32_t RA8889_common::tft_boxGet(uint32_t vPageAddr, uint16_t x0, uint16_t y0,
          uint16_t x1, uint16_t y1, uint16_t dx0, uint16_t dy0) {
...
void RA8889_common::_fontWrite(const uint8_t *buffer, uint16_t len) {
    if (_use_default) {
...
// Draw a character
void RA8889_common::drawChar(int16_t x, int16_t y, unsigned char c,
...
void RA8889_common::setFont(const ILI9341_t3_font_t &f) {
    _use_default = 0;
    ...
    if (font && font->version == 23) {
        ...
        if (_TXTForeColor == _TXTBackColor)
            _TXTBackColor = (_TXTForeColor == 0x0000) ? 0xFFFF : 0x0000;
    }
}

// Maybe support GFX Fonts as well?
void RA8889_common::setFont(const GFXfont *f) {
    _use_default = 0;
...
void RA8889_common::drawFontChar(unsigned int c) {
...
int16_t RA8889_common::strPixelLen(const char *str) {
...
void RA8889_common::charBounds(char c, int16_t *x, int16_t *y,
...
void RA8889_common::getTextBounds(const uint8_t *buffer, uint16_t len, int16_t x, int16_t y,
...
void RA8889_common::getTextBounds(const char *str, int16_t x, int16_t y,
                                  int16_t *x1, int16_t *y1, uint16_t *w, uint16_t *h) {
...
void RA8889_common::getTextBounds(const String &str, int16_t x, int16_t y,
                                  int16_t *x1, int16_t *y1, uint16_t *w, uint16_t *h) {
    if (str.length() != 0) {
...
void RA8889_common::drawFontBits(bool opaque, uint32_t bits, uint32_t numbits, int32_t x, int32_t y, uint32_t repeat) {
...
void RA8889_common::drawGFXFontChar(unsigned int c) {
    ...
    if (_backTransparent) {
        ...
        if (x_end > _displayclipx2)
            x_end = _displayclipx2;
        // If we get here and
        if (_gfx_last__cursorY != (_cursorY + _originy))
            _gfx_last_char_x_write = 0;
            ...
            if ((y >= _displayclipy1) && (y < _displayclipy2)) {
                for (int16_t xx = x_start; xx < x_end; xx++) {
                    if (xx >= _displayclipx1) {
                        combineAndDrawPixel(xx, y, gfxFontLastCharPosFG(xx, y) ? _gfx_last_char_textcolor : (xx < x_offset_cursor) ? _gfx_last_char_textbgcolor
                                                                                                                                   : _TXTBackColor);
                    }
                }
            }
                        ...
                        for (uint8_t xts = 0; xts < textsize_x; xts++) {
                            if ((x >= _displayclipx1) && (x < _displayclipx2)) {
                                if (bits & 0x80)
                                    ...
                                    combineAndDrawPixel(x, y, gfxFontLastCharPosFG(x, y) ? _gfx_last_char_textcolor : (x < x_offset_cursor) ? _gfx_last_char_textbgcolor
                                                                                                                                            : _TXTBackColor);
                            }
...
bool RA8889_common::gfxFontLastCharPosFG(int16_t x, int16_t y) {
    ...
    if (y < (_gfx_last__cursorY + (yo * textsize_y)))
        return false; // above
    if (y >= (_gfx_last__cursorY + (yo + h) * textsize_y))
        return false; // below
...
void RA8889_common::setTextSize(uint8_t s_x, uint8_t s_y) {
    textsize_x = (s_x > 0) ? s_x : 1;
    textsize_y = (s_y > 0) ? s_y : 1;
    _gfx_last_char_x_write = 0; // Don't use cached data here
}

void RA8889_common::drawFontPixel(uint8_t alpha, uint32_t x, uint32_t y) {
...
void RA8889_common::Pixel(int16_t x, int16_t y, uint16_t color) {
    x += _originx;
    y += _originy;
...
uint32_t RA8889_common::fetchbit(const uint8_t *p, uint32_t index) {
    if (p[index >> 3] & (1 << (7 - (index & 7))))
        return 1;
    return 0;
}

uint32_t RA8889_common::fetchbits_unsigned(const uint8_t *p, uint32_t index, uint32_t required) {
    uint32_t val = 0;
    do {
        uint8_t b = p[index >> 3];
...
uint32_t RA8889_common::fetchbits_signed(const uint8_t *p, uint32_t index, uint32_t required) {
...
uint32_t RA8889_common::fetchpixel(const uint8_t *p, uint32_t index, uint32_t x) {
    // The byte
    uint8_t b = p[index >> 3];
...
*/
/**************************************************************************/
void RA8889_common::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
	x+=_originx;
	y+=_originy;
...
*/
/**************************************************************************/
void RA8889_common::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
	x+=_originx;
	y+=_originy;
...
*/
/**************************************************************************/
void RA8889_common::setActiveWindow(int16_t XL, int16_t XR, int16_t YT, int16_t YB) {
...
*/
/**************************************************************************/
void RA8889_common::setActiveWindow(void) {
    _activeWindowXL = 0;
    _activeWindowXR = _width;
    _activeWindowYT = 0;
    _activeWindowYB = _height;
...
*/
/**************************************************************************/
void RA8889_common::_updateActiveWindow(bool full) {
    if (full) {
        // X
        activeWindowXY(0, 0);
        activeWindowWH(_width, _height);
...
*/
/**************************************************************************/
void RA8889_common::_textPosition(int16_t x, int16_t y, bool update) {
    lcdRegDataWrite(RA8889_F_CURX0, x, false);      // 63h
    lcdRegDataWrite(RA8889_F_CURX1, x >> 8, false); // 64h
    lcdRegDataWrite(RA8889_F_CURY0, y, false);      // 65h
    lcdRegDataWrite(RA8889_F_CURY1, y >> 8, true);  // 66h
...
void RA8889_common::_setFNTdimensions(uint8_t index) {
...
size_t RA8889_common::write(uint8_t c) {
    return write(&c, 1);
}

size_t RA8889_common::write(const uint8_t *buffer, size_t size) {
    if (_use_default) {
        size_t cb = size;
        // Lets try to handle some of the special font centering code that was done for default fonts.
        if (_absoluteCenter || _relativeCenter) {
...
//************************************************//
size_t RA8889_common::rawPrint(uint8_t text) {
    update_tft(text);
    _cursorX += (_FNTwidth * _scaleX);
    switch (_FNTwidth) {
    case 12: // Font width is 12, 1024 / 12 =  85.3, have to
...
/* Turn RA8889 text rotate mode ON/OFF (True = ON)              */
//**************************************************************//
void RA8889_common::setRotation(uint8_t rotation) { // rotate text and graphics
    ...
    case 0:
        ...
        _portrait = false;
        VSCAN_T_to_B();
        macr_settings = RA8889_DIRECT_WRITE << 6 | RA8889_READ_MEMORY_LRTB << 4 | RA8889_WRITE_MEMORY_LRTB << 1;
        break;
    case 1:
        _portrait = true;
        ...
        VSCAN_B_to_T();
        macr_settings = RA8889_DIRECT_WRITE << 6 | RA8889_READ_MEMORY_LRTB << 4 | RA8889_WRITE_MEMORY_RLTB << 1;
        break;
    case 2:
        _width = SCREEN_WIDTH;
        _height = SCREEN_HEIGHT;
    ...
    case 3:
        ...
        macr_settings = RA8889_DIRECT_WRITE << 6 | RA8889_READ_MEMORY_LRTB << 4 | RA8889_WRITE_MEMORY_LRTB << 1;
        // VSCAN_T_to_B();
        // macr_settings = RA8889_DIRECT_WRITE<<6|RA8889_READ_MEMORY_LRTB<<4|RA8889_WRITE_MEMORY_BTLR<<1;
        break;
    }
    lcdRegWrite(RA8889_MACR); // 02h
...
*/
/**************************************************************************/
uint8_t RA8889_common::getRotation() {
    return _rotation;
}
...
/* Turn RA8889 text rotate mode ON/OFF (True = ON)              */
//**************************************************************//
void RA8889_common::textRotate(boolean on) {
    if (on) {
        lcdRegDataWrite(RA8889_CCR1, RA8889_TEXT_ROTATION << 4); // cdh
    } else {
        lcdRegDataWrite(RA8889_CCR1, RA8889_TEXT_NO_ROTATION << 4); // cdh
    }
}

void RA8889_common::MemWrite_Left_Right_Top_Down(void) {
    ...
    temp = lcdRegDataRead(RA8889_MACR);
    Serial.println(temp, BIN);
    temp &= cClrb2;
    temp &= cClrb1;
    Serial.println(temp, BIN);
    lcdRegDataWrite(RA8889_MACR, temp);

    temp = lcdRegDataRead(RA8889_MACR);
    Serial.println(temp, BIN);
}

void RA8889_common::MemWrite_Right_Left_Top_Down(void) {
    /* Host Write Memory Direction (Only for Graphic Mode)
...
void RA8889_common::MemWrite_Top_Down_Left_Right(void) {
    /* Host Write Memory Direction (Only for Graphic Mode)
    ...
    // temp = RA8889_DIRECT_WRITE<<6|RA8889_READ_MEMORY_BTLR<<4|RA8889_WRITE_MEMORY_BTLR<<1;
    temp = RA8889_DIRECT_WRITE << 6 | RA8889_READ_MEMORY_LRTB << 4 | RA8889_WRITE_MEMORY_TBLR << 1;
    //  Serial.println(temp, BIN);
    lcdRegDataWrite(RA8889_MACR, temp);
...
void RA8889_common::MemWrite_Down_Top_Left_Right(void) {
    /* Host Write Memory Direction (Only for Graphic Mode)
    ...
    // temp = RA8889_DIRECT_WRITE<<6|RA8889_READ_MEMORY_BTLR<<4|RA8889_WRITE_MEMORY_BTLR<<1;
    temp = RA8889_DIRECT_WRITE << 6 | RA8889_READ_MEMORY_LRTB << 4 | RA8889_WRITE_MEMORY_BTLR << 1;
    //  Serial.println(temp, BIN);
    lcdRegDataWrite(RA8889_MACR, temp);
...
void RA8889_common::VSCAN_T_to_B(void) {
    /*
    ...
    */
    ...
    temp = lcdRegDataRead(RA8889_DPCR);
    temp &= cClrb3;
    lcdRegDataWrite(RA8889_DPCR, temp);
}

void RA8889_common::VSCAN_B_to_T(void) {
    /*
    Vertical Scan direction
    0 : From Top to Bottom
    1 : From bottom to Top
    ...
    */
    ...
    temp_in = temp = lcdRegDataRead(RA8889_DPCR);
    temp |= cSetb3;
    lcdRegDataWrite(RA8889_DPCR, temp);
...
/**************************************************************************/
/*!
  The FT5206 Capacitive Touch driver uses a different INT pin than RA8889_common
...
*/
/**************************************************************************/
void RA8889_common::useCapINT(const uint8_t INTpin, const uint8_t RSTPin) {
    _intCTSPin = INTpin;
    _rstCTSPin = RSTPin;
...
*/
/**************************************************************************/
void RA8889_common::cts_isr(void) {
    _FT5206_INT = true;
}
...
*/
/**************************************************************************/
void RA8889_common::enableCapISR(bool force) {
...
*/
/**************************************************************************/
void RA8889_common::_disableCapISR(void) {
    if (_useISR) {
...
*/
/**************************************************************************/
void RA8889_common::printTSRegisters(Print &pr, uint8_t start, uint8_t count) {
    // guessing. from registers pdf
    _wire->beginTransmission(_ctpAdrs);
    _wire->write(start);
    _wire->endTransmission(_ctpAdrs);

    pr.println("\nRA8889 CTP Registers");
...
*/
/**************************************************************************/
bool RA8889_common::touched(bool safe) {
...
void RA8889_common::setTouchLimit(uint8_t limit) {
    if (limit > 5)
        limit = 5; // max 5 allowed
    _maxTouch = limit;
}

uint8_t RA8889_common::getTouchLimit(void) {
    return _maxTouch;
}
...
void RA8889_common::_initializeFT5206(void) {
    uint8_t i;
    for (i = 0x80; i < 0x89; i++) {
...
*/
/**************************************************************************/
void RA8889_common::_sendRegFT5206(uint8_t reg, const uint8_t val) {
    _wire->beginTransmission(_ctpAdrs);
    _wire->write(reg);
    _wire->write(val);
    _wire->endTransmission(_ctpAdrs);
...
*/
/**************************************************************************/
void RA8889_common::updateTS(void) {
    ...
    if (_currentTouches > _maxTouch)
        _currentTouches = _maxTouch;
    _gesture = _cptRegisters[0x01];
    if (_maxTouch < 2)
        _gesture = 0;
    uint8_t temp = _cptRegisters[0x03];
    _currentTouchState = 0;
    if (!bitRead(temp, 7) && bitRead(temp, 6))
        _currentTouchState = 1; // finger up
    if (bitRead(temp, 7) && !bitRead(temp, 6))
        _currentTouchState = 2; // finger down
}
...
*/
/**************************************************************************/
uint8_t RA8889_common::getTScoordinates(uint16_t (*touch_coordinates)[2]) {
    uint8_t i;
    if (_currentTouches < 1)
        return 0;
...
*/
/**************************************************************************/
uint8_t RA8889_common::getTouchState(void) {
...
*/
/**************************************************************************/
uint8_t RA8889_common::getTouches(void) {
    return _currentTouches;
}
...
*/
/**************************************************************************/
uint8_t RA8889_common::getGesture(void) {
...
bool RA8889_common::setCanvasRegion(uint32_t address, uint16_t width) {
    if (address & 0x3)
    ...
    // writeReg32(RA8889_REG_CVSSA0, address);
    canvasImageStartAddress(address);
    // uint8_t aw_color = readReg(RA8889_REG_AW_COLOR);
    uint8_t aw_color = lcdRegDataRead(RA8889_AW_COLOR);

    if (width) {
        aw_color &= 0xFB; // Block mode
        // writeReg16(RA8889_REG_CVS_IMWTH0, width);
        canvasImageWidth(width);
    } else {
        aw_color |= 0x04; // Linear mode
    }

    // writeReg(RA8889_REG_AW_COLOR, aw_color);
    lcdRegDataWrite(RA8889_AW_COLOR, aw_color);

    return true;
}

bool RA8889_common::setCanvasWindow(uint16_t x, uint16_t y, uint16_t width, uint16_t height) {
    if (x + width > 8188)
        return false;
...
bool RA8889_common::setDisplayRegion(uint32_t address, uint16_t width) {
    if (address & 0x3)
        return false; // Address must me multiple of 4
...
bool RA8889_common::setDisplayOffset(uint16_t x, uint16_t y) {
    if (x > 8188)
        return false;
    else if (y > 8191)
        return false;
